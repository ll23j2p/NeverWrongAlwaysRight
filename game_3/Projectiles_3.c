...
float DegToRad(float degrees) { return degrees * 3.14159265f / 180.0f; }
...
void CalculateRayEnd(float start_x, float start_y, float angle, float *end_x,
                     float *end_y) {
  ...
  for (float dist = 0.0f; dist <= RAY_LENGTH; dist += RAY_STEP) {
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
      ...
      if (CheckAABB(test_x, test_y, 1.0f, 1.0f, enemy_left, enemy_top,
...
// "try shoot" function called by update player, currently we only shoot once
// per button press
